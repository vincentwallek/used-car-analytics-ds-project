...
st.markdown(f"""
    <style>
        ...
        /* --- Selectbox dropdown popup (BaseWeb) --- */
        ...
        /* Ensure the select control text is readable */
        [data-baseweb="select"] {{
            background-color: {T['input_bg']} !important;
        }}
        ...
        [data-baseweb="button-stepper"]:hover {{
            background-color: {T['accent']} !important;
            color: #ffffff !important;
        }}
...
# get_market_data moved to helpers.py
...
# predict_price and run_ml_prediction moved to helpers.py
...
# formatting logics moved to helpers.py
...
def view_app():
    ...
    db_data = get_market_data(market, supabase)
                        ...
                        if response_msg.tool_calls:
                            ...
                            for tool_call in response_msg.tool_calls:
                                if tool_call.function.name == "run_ml_prediction":
                                    ...
                                    tool_result = run_ml_prediction(trained_models, **args)
...
def _render_de_form_fields(enc_cats, role, show_advanced):
    ...
    if sel_trim != "Alle" and brand and model_name:
        db_data = get_market_data("DE", supabase)
...
# _collect_inputs moved to helpers.py
